...
if configuration_mode == 13:
    ...
    mat = loadmat(
        "./{}_{}_{}_{}_DNN_training_dataset_{}.mat".format(
    ...
    model = Sequential(
        [
            Dense(
                units=int(hidden_layer1),
                activation="relu",
                input_dim=int(dnn_input),
                kernel_initializer=init,
                bias_initializer=init,
            ),
            Dense(
                units=int(hidden_layer2),
                activation="relu",
                kernel_initializer=init,
                bias_initializer=init,
            ),
            Dense(
                units=int(hidden_layer3),
                activation="relu",
                kernel_initializer=init,
                bias_initializer=init,
            ),
            Dense(
                units=int(dnn_output), kernel_initializer=init, bias_initializer=init
            ),
        ]
    )
    ...
    RLearningRate = ReduceLROnPlateau(
        monitor="val_loss", factor=0.01, patience=20, min_lr=0.000001, min_delta=0.002
    )
    ...
    for j in SNR_index:
        mat = loadmat(
            "./{}_{}_{}_{}_DNN_testing_dataset_{}.mat".format(
                mobility, channel_model, modulation_order, scheme, j
            )
        )
        Dataset = mat["DNN_Datasets"]
        Dataset = Dataset[0, 0]
        ...
        result_path = "./{}_{}_{}_{}_DNN_Results_{}.pickle".format(
            mobility, channel_model, modulation_order, scheme, j
        )
        dest_name = "./{}_{}_{}_{}_DNN_Results_{}.mat".format(
            mobility, channel_model, modulation_order, scheme, j
        )
